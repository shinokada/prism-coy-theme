...
var createHead = function (data, vfile) {
    ...
    // <meta>
    {
        ...
        for (var _i = 0, meta_1 = meta; _i < meta_1.length; _i++) {
            ...
            head.push({ type: 'text', value: '\n' }, hastscript_1.default('meta', props));
        }
        head.push({ type: 'text', value: '\n' });
    }
    // <link>
    if (data.link) {
        for (var _a = 0, _b = data.link; _a < _b.length; _a++) {
            ...
            var props = createProperties(attributes);
            head.push({ type: 'text', value: '\n' }, hastscript_1.default('link', props));
        }
        head.push({ type: 'text', value: '\n' });
    }
    // <script>
    if (data.script) {
        for (var _c = 0, _d = data.script; _c < _d.length; _c++) {
            ...
            var props = createProperties(attributes);
            head.push({ type: 'text', value: '\n' }, hastscript_1.default('script', props));
        }
        head.push({ type: 'text', value: '\n' });
    }
    return head;
...
var createBody = function (metadata, tree) {
    ...
    return hastscript_1.default('body', props, contents);
};
/**
...
var createHTML = function (metadata, tree, vfile) {
    ...
    return hastscript_1.default('html', props, [
        { type: 'text', value: '\n' },
        hastscript_1.default('head', createHead(metadata, vfile)),
        { type: 'text', value: '\n' },
...
var mdast = function (data) { return function (tree, vfile) {
    return {
        type: 'root',
        children: [
            { type: 'doctype', name: doctype_1.default(5) },
            { type: 'text', value: '\n' },
